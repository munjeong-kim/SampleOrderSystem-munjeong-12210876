...
def test_재고가_충분해서_즉시_CONFIRMED로_승인되면_재고가_주문수량만큼_차감된다(tmp_path, mocker):
    view = mocker.MagicMock()
    sample_repository, order_repository, production_queue_repository = _make_repositories(tmp_path)
    sample = Sample(
        sample_id="S-001", name="실리콘 웨이퍼-8인치", avg_production_time=1.0, yield_rate=0.9, stock_quantity=100
    ...
    order = order_repository.read_one("ORD-20260416-0001")
    assert order.status == OrderStatus.CONFIRMED
    assert sample_repository.read_one("S-001").stock_quantity == 40  # 100 - 60


def test_이중_할당_버그가_재현되지_않는다_순차_승인시_재고가_반영되어_두번째_주문은_PRODUCING된다(
    tmp_path, mocker
):
    view = mocker.MagicMock()
    sample_repository, order_repository, production_queue_repository = _make_repositories(tmp_path)
    ...
    assert sample_repository.read_one("S-001").stock_quantity == 50  # 100 - 50

    controller.approve("ORD-20260416-0002")
    order2 = order_repository.read_one("ORD-20260416-0002")
    # 남은 재고(50) < 주문 수량(80) 이므로 이제는 PRODUCING으로 정확히 분기되어야 한다

...
def test_재고가_충분해서_즉시_CONFIRMED로_승인되어도_재고는_차감되지_않는다(tmp_path, mocker):
    view = mocker.MagicMock()
    sample_repository, order_repository, production_queue_repository = _make_repositories(tmp_path)
    sample = Sample(
        sample_id="S-001", name="실리콘 웨이퍼-8인치", avg_production_time=1.0, yield_rate=0.9, stock_quantity=100
    ...
    sample_repository.create(sample)
    order_repository.create(
        Order(order_id="ORD-20260416-0001", sample_id="S-001", customer_name="홍길동", quantity=60)
    )
    controller = OrderController(view, order_repository, sample_repository, production_queue_repository)
    ...
    order = order_repository.read_one("ORD-20260416-0001")
    assert order.status == OrderStatus.CONFIRMED
    # 재고 차감은 이제 출고(RELEASE) 시점에만 발생하므로, 승인 시점에는 재고가 그대로여야 한다.
    assert sample_repository.read_one("S-001").stock_quantity == 100


def test_이중_할당_버그가_재현되지_않는다_가용재고_기준으로_두번째_주문은_PRODUCING된다(
    tmp_path, mocker
):
    view = mocker.MagicMock()
    sample_repository, order_repository, production_queue_repository = _make_repositories(tmp_path)
    ...
    order_repository.create(
        Order(order_id="ORD-20260416-0001", sample_id="S-001", customer_name="홍길동", quantity=50)
    )
    order_repository.create(
        Order(order_id="ORD-20260416-0002", sample_id="S-001", customer_name="김철수", quantity=80)
    )
    controller = OrderController(view, order_repository, sample_repository, production_queue_repository)

    controller.approve("ORD-20260416-0001")
    order1 = order_repository.read_one("ORD-20260416-0001")
    assert order1.status == OrderStatus.CONFIRMED
    # 물리적 재고는 출고 전까지 변하지 않는다(승인 단계에서는 차감하지 않음).
    assert sample_repository.read_one("S-001").stock_quantity == 100

    controller.approve("ORD-20260416-0002")
    order2 = order_repository.read_one("ORD-20260416-0002")
    # 가용재고 = 100(재고) - 50(이미 CONFIRMED된 첫 주문) = 50 < 80 이므로 PRODUCING이어야 한다.
    assert order2.status == OrderStatus.PRODUCING


def test_부족분_계산은_CONFIRMED_주문을_반영한_가용재고_기준으로_이루어진다(tmp_path, mocker):
    view = mocker.MagicMock()
    sample_repository, order_repository, production_queue_repository = _make_repositories(tmp_path)
    sample_repository.create(
        Sample(
            sample_id="S-001", name="실리콘 웨이퍼-8인치", avg_production_time=10.0, yield_rate=0.5,
            stock_quantity=100,
        )
    )
    order_repository.create(
        Order(
            order_id="ORD-20260416-0001", sample_id="S-001", customer_name="홍길동", quantity=60,
            status=OrderStatus.CONFIRMED,
        )
    )
    order_repository.create(
        Order(order_id="ORD-20260416-0002", sample_id="S-001", customer_name="김철수", quantity=100)
    )
    controller = OrderController(view, order_repository, sample_repository, production_queue_repository)
    ...
    # 가용재고 = 100 - 60(기존 CONFIRMED) = 40, 부족분 = 100 - 40 = 60
    # 실생산량 = ceil(60 / 0.5) = 120, 총생산시간 = 10.0 * 120 = 1200.0
    assert job.quantity == 120
    assert job.total_seconds == 1200.0

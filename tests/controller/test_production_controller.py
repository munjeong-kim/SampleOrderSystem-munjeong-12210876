...
def test_여러_작업이_연쇄로_완료_시간을_지났으면_모두_순서대로_처리된다(tmp_path, mocker):
    ...
    # 각 주문의 quantity(10/20/30)와 job.quantity(10/20/30)가 동일하므로 순증감이 0이 된다
    assert sample_repository.read_one("S-001").stock_quantity == 0
    assert sample_repository.read_one("S-002").stock_quantity == 0
    assert sample_repository.read_one("S-003").stock_quantity == 0
